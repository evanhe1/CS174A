...
export class Assignment3 extends Scene {
  constructor() {
    ...
    this.materials = {
      ...
      planet1_surface: new Material(new defs.Phong_Shader(), {
        ambient: 0,
        diffusivity: 0.6,
        color: hex_color("#a9a9a9"),
      }),
      planet2_surface_phong: new Material(new defs.Phong_Shader(), {
        ambient: 0,
        diffusivity: 0.6,
        color: hex_color("#80ffff"),
      }),
      planet2_surface_gouraud: new Material(new Gouraud_Shader(), {
        diffusivity: 0.6,
        color: hex_color("#80ffff"),
      }),
      planet3_surface: new Material(new defs.Phong_Shader(), {
        color: hex_color("#b08040"),
      }),
      planet4_surface: new Material(new defs.Phong_Shader(), {
        color: hex_color("#000473"),
      }),
  ...
  display(context, program_state) {
    ...
    if (this.attached) {
      let desired =
        this.attached() === null
          ...
          : Mat4.inverse(this.attached().times(Mat4.translation(0, 0, 5)));
    ...
    let r = (51 * -Math.cos((Math.PI / 10) * t) + 204) / 255;
    let g = (107.5 * -Math.cos((Math.PI / 10) * t) + 147.5) / 255;
    let b = g;
    let scaler = -Math.cos((Math.PI / 10) * t) + 2;
    const model_transform = Mat4.identity().times(
      Mat4.scale(scaler, scaler, scaler)
    );
    const light_position = vec4(0, 0, 0, 1);
    program_state.lights = [
      new Light(light_position, color(r, g, b, 1), 10 ** scaler),
    ];
    /*
    ...
    this.shapes.sphere.draw(
      ...
      program_state,
      model_transform,
      this.materials.test.override({ ambient: 1, color: color(r, g, b, 1) })
    );

    const angle1 = ((t % 10) / 10) * 2 * Math.PI;
    const planet_1 = (this.planet_1 = Mat4.identity()
      .times(Mat4.rotation(angle1, 0, 1, 0))
    ...
    this.shapes.planet1.draw(
      ...
      program_state,
      planet_1,
      this.materials.planet1_surface
    );

    const angle2 = ((t % 15) / 15) * 2 * Math.PI;
    const planet_2 = (this.planet_2 = Mat4.identity()
      .times(Mat4.rotation(angle2, 0, 1, 0))
      .times(Mat4.translation(8, 0, 0)));

    let even = Math.floor(t) % 2 === 0;
    if (even)
      this.shapes.planet2.draw(
        context,
        program_state,
        planet_2,
        this.materials.planet2_surface_phong
      );
    else
      this.shapes.planet2.draw(
        context,
        program_state,
        planet_2,
        this.materials.planet2_surface_gouraud
      );
    ...
    const angle4 = ((t % 25) / 25) * 2 * Math.PI;
    const planet_4 = (this.planet_4 = Mat4.identity()
      .times(Mat4.rotation(angle4, 0, 1, 0))
    ...
    this.shapes.planet4.draw(
      ...
      program_state,
      planet_4,
      this.materials.planet4_surface
    );

    const angle_moon = ((t % 2.5) / 2.5) * 2 * Math.PI;
    const moon = (this.moon = planet_4
      .times(Mat4.rotation(angle_moon, 0, 1, 0))
    ...
    this.shapes.moon.draw(
      ...
      program_state,
      moon,
      this.materials.test.override({ color: hex_color("#CA27D5") })
    );
  }
}
...
class Ring_Shader extends Shader {
  ...
  fragment_glsl_code() {
    ...
    return (
      ...
      `
        void main(){
            float scalar = sin(29.0 * distance(center, point_position));
            gl_FragColor = scalar * vec4(.808, .588, .149, 1);
        }`
    );
  }
}
